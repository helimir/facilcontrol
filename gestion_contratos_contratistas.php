                                ...
                                <table class="table footable " data-page-size="10" data-filter="#filter">
                                   <thead style="background:#010829;color:#fff">
                                    ...
                                    <?php
                                      ...
                                      $query_ta=mysqli_query($con,"select a.*, t.*, c.cargo, p.cargos as cargos_perfil, o.perfiles from trabajadores_asignados as a LEFT JOIN trabajador as t ON t.idtrabajador=a.trabajadores LEFT JOIN cargos as c ON c.idcargo=a.cargos LEFT JOIN perfiles_cargos as p ON p.contrato=a.contrato LEFT JOIN contratos as o ON o.id_contrato=a.contrato  where a.estado!=2 and a.contrato='".$_SESSION['contrato']."' and a.mandante='".$_SESSION['mandante']."' ");
                                         ...
                                         <tr>
                                            ...
                                            <td style="text-align: center;">
                                             <?php if ($row['verificados']==0) { ?>    
                                                    <button title="Trabajador No Validado" class="btn btn-danger btn-xs btn-block" onclick="doc_validar(<?php echo $row['idtrabajador'] ?>,<?php echo $row['cargos'] ?>,<?php echo $result_pc['id_perfil'] ?>)">Gesti&oacute;n de Documentos</button>
                                             <?php } else { 
                                                      if ($row['verificados']==1) { ?> 
                                                        <button title="Trabajador Validado" class="btn btn-success btn-xs btn-block" onclick="doc_validar(<?php echo $row['idtrabajador'] ?>,<?php echo $row['cargos'] ?>,<?php echo $result_pc['id_perfil'] ?>)">Gesti&oacute;n de Documentos</button>
                                                <?php } else {   ?>
                                                        <button title="Validacion En Proceso" class="btn btn-warning btn-xs btn-block" onclick="doc_validar(<?php echo $row['idtrabajador'] ?>,<?php echo $row['cargos'] ?>,<?php echo $result_pc['id_perfil'] ?>)">Gesti&oacute;n de Documentos</button>    
                                                <?php }    
                                                   } ?>
                                            </td>
