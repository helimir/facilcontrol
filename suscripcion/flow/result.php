...
if (empty($_GET['token']) ) { 
    ...
    $query=mysqli_query($con,"select * from registro where token='".$_GET['token']."' ");
    ...
    if ($estado==0) {
        ...
        $fin=$response2['subscription_end'];
        $periodo_inicio=$response2['period_start'];
        $periodo_fin=$response2['period_end'];
        ...
        $subscriptionID=$response2['subscriptionId'];

        $query_suscripcion=mysqli_query($con,"INSERT INTO suscripciones (subscriptionId,custorm_id,plan,inicio,fin,periodo_inicio,periodo_fin,proxima_factura,dias_hasta_vencimiento,creado,estado,morosidad) VALUES ('$subscriptionID','$id_custorm','$plan','$inicio','$fin','$periodo_inicio','$periodo_fin','$proxima_factura','$dias_hasta_vencimiento','$fecha','$estado','$mororidad')  ");

        # si se creo la suscripcion
        #if ($query_suscripcion) {
            $invoices=$response2['invoices'];
            foreach($invoices as $invoice) {
                mysqli_query($con,"INSERT INTO facturas (id_invoice,subscriptionId,custorm_id,asunto,currency,amount,period_start,period_end,attemp_count,attemped,next_attemp_date,due_date,creado,estado) VALUES ('".$invoice['id']."','".$invoice['subscriptionId']."','".$invoice['customerId']."','".$invoice['subject']."','".$invoice['currency']."','".$invoice['amount']."','".$invoice['period_star']."','".$invoice['period_end']."','".$invoice['attemp_count']."','".$invoice['attemped']."','".$invoice['next_attemp_date']."','".$invoice['due_date']."','".$invoice['created']."','".$invoice['status']."' ) ");            
            }
        #}

        #actualiza datos de tarjeta y metodo de pago en registro
        mysqli_query($con,"update registro set subscriptionId='$subscriptionID', modo_pago='$modo_pago', numero_tarjeta='$ultimos_numeros', tipo_tarjeta='$tipo_tarjeta', estado=1 where id_registro='$id_cliente' ");
    }
                                            ...
                                            <div class="row">
                                                <div class="col-lg-12">
                                                    <div  class="ibox">
                                                        <div style="background:#262626;border-radius:10px;" class="ibox-content" id="">
                                                                <div class="row">
                                                                    <div style="font-size:15px;color:#fff;text-align:center" class="col-lg-12">
                                                                        <label style="font-size:18px;" class="form-label">TOKEN NO VÁLIDO. SUSCRIPCION YA PROCESADA.<b><?php echo $cliente ?></b></label>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="col-lg-12">
                                                    <div  class="ibox">
                                                        <div style="background:#262626;border-radius:10px;" class="ibox-content" id="">
                                                            ...
                                                            <div class="row">
                                                                <div style="font-size:15px;color:#fff;text-align:center" class="col-lg-12">
                                                                    <label style="font-size:18px;" class="form-label">TOKEN NO VÁLIDO. SUSCRIPCION YA PROCESADA.</b></label>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
