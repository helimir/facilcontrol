...
if (isset($_SESSION['usuario']) and $_SESSION['nivel']==3  ) {    
    ...
    function agregar_zip($dir, $zip,$rut) {
        ...
        $dir_f='documentos_validados_trabajador_'.$rut.'/';
        if (is_dir($dir)) {
            ...
            if(!$_FILES["foto"]["error"]>0){
                        ...
                        $archivo_contratista = $carpeta_contratista.$nombre_contratista;
                  		...
                  		if($resultado){
                            ...
                            $destino_c='../doc/validados/'.$mandante.'/'.$contratista.'/contrato_'.$contrato.'/'.$rut.'/'.$codigo.'/foto_'.$contratista.'_'.$rut.'.jpg';
                            $destino_t='../doc/temporal/'.$mandante.'/'.$contratista.'/contrato_'.$contrato.'/'.$rut.'/foto_'.$contratista.'_'.$rut.'.jpg';
                            ...
                            mysqli_query($con,"update trabajadores_asignados set  url_foto='$url_contrato' where trabajadores='".$trabajador."' and contrato='".$contrato."'  ");

                            mysqli_query($con,"delete from notificaciones  where contratista='$contratista' and contrato='$contrato' and item='Trabajador Sin Foto' and trabajador='$trabajador' " );


                            #crear zip con foto
                            $ruta='../doc/validados/'.$mandante.'/'.$contratista.'/contrato_'.$contrato.'/'.$rut.'/'.$codigo.'/';
                            $rutazip='../doc/validados/'.$mandante.'/'.$contratista.'/contrato_'.$contrato.'/'.$rut.'/'.$codigo.'/zip/';

                            
                            $zip = new ZipArchive();
                            # crea archivo zip
                            $archivoZip = "documentos_validados_trabajador_$rut.zip";
                            ...
                            $url_foto='doc/validados/'.$mandante.'/'.$contratista.'/contrato_'.$contrato.'/'.$rut.'/'.$codigo.'/foto_'.$contratista.'_'.$rut.'.jpg';
                            mysqli_query($con,"update trabajadores_acreditados set url_foto='$url_foto' where trabajador='$trabajador' ");   
                            echo 0;   
                        } else {
