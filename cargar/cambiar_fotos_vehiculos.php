                        ...
                        $archivo_contratista = $carpeta_contratista.$nombre_contratista;
                  		...
                  		if($resultado){
                            ...
                            if (copy($origen_c,$destino_c)) {
                                ...
                                $query_comentario=mysqli_query($con,"select * from comentarios where leer_contratista=0 and leer_mandante=0 and doc='Foto del vehiculo' and contratista='$contratista' and mandante='$mandante' and trabajador='$trabajador' and contrato='$contrato' ");
                                ...
                                if ($existe_comentario>0) {
                                    ...
                                    $mensaje="El contratista <b>$nom_contratista</b> envio el documento <b>Foto del vehículo</b> del vehiculo <b>$nom_trabajador</b>, contrato <b>$nom_contrato</b> para ser revisada.";
                                    $usuario=$_SESSION['usuario'];
                                    $accion="Revisar foto de vehiculo reenviada";
                                    $url="verificar_documentos_vehiculos_mandante.php";
                                    
                                    mysqli_query($con,"insert into notificaciones (item,nivel,envia,recibe,mensaje,accion,fecha,usuario,url,tipo,mandante,contratista,control,contrato,trabajador,documento,perfil) values ('$item','$nivel','$envia','$recibe','$mensaje','$accion','$date','$usuario','$url','$tipo','".$_SESSION['mandante']."','$contratista','Foto del vehiculo','$contrato','".$trabajador."','Foto del vehiculo','$perfil') ");
                                }
                                
                                mysqli_query($con,"update comentarios set leer_contratista=1,leer_mandante=1, estado='1' where  doc='Foto del vehiculo' and contratista='$contratista' and mandante='$mandante' and trabajador='$trabajador' and contrato='$contrato' ");
                                mysqli_query($con,"delete from notificaciones  where contrato='$contrato' and contratista='$contratista' and item='Observacion Documento Vehiculo' and control='Foto del vehiculo' and trabajador='$trabajador' ");                                  
