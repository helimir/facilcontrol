...
$query_mensual=mysqli_query($con,"select m.*, t.*, c.nombre_contrato, a.razon_social from mensuales_trabajador as m Left Join contratos as c On c.id_contrato=m.contrato Left Join trabajador as t On t.idtrabajador=m.trabajador Left Join mandantes as a On a.id_mandante=m.mandante where m.mensuales='".$_GET['mensual']."' and t.idtrabajador='".$_GET['id']."' ");
...
<script src="js\jquery-3.1.1.min.js"></script>
  ...
  function cargar_doc_mensual(trabajador,doc,contrato,mensual,mandante,mes,id){
               //alert(doc);
               var idnombre= 'carga_dm_trabajador'+id;
               var fileInput = document.getElementById(idnombre);
                        ...
                        $.ajax({
                                ...
                                success: function(response) {
                                    if (response==0) {                                        
                                        ...
                                        setTimeout(
                                        function() {
                        	               window.location.href='verificar_documentos_contratista_mensual.php?id='+trabajador+'&contrato='+contrato+'&mensual='+mensual+'&mandante='+mandante;
                                        },1500);
                                    } else {                                       
                                       swal({
                                   ...
                                   <tbody>
                                    ...
                                    <?php          
                                         ...
                                         foreach ($documentos as $row) {
                                            ...
                                            $query_com=mysqli_query($con,"select * from comentarios where id_obs='".$result_obs['id_obs']."' and doc='".$result_d['documento']."'  order by id_com desc  ");
                                            $result_com=mysqli_fetch_array($query_com);
                                            $list_com=$result_com['comentarios'];
                                              
                                            $arreglo=array(".png",".jpg",".jpeg",".pdf");
                                            for ($j=0;$j<=3;$j++) {
                                                $carpeta='doc/mensuales/'.$result_mensual['contratista'].'/'.$result_mensual['contrato'].'/trabajadores/'.$result_mensual['mes'].'-'.$result_mensual['year'].'/'.$result_mensual['rut'].'/'.$result_d['documento'].'_'.$result_mensual['rut'].$arreglo[$j];;
                                        ...
                                        <?php 
                                            ...
                                            if ($archivo_existe) { ?>
                                                       ...
                                                       <!-- documento -->
                                                       <td style=""><a href="<?php echo $carpeta ?>" target="_blank"><?php echo $result_d['documento'] ?></a></td>
                                                       <?php  
                                                       # si esta verificado 
                                                       if ($list_veri[$i]==0 ) { ?>
                                                        ...
                                                        <td style="text-align:center">
                                                           <!-- <button class="btn btn-sm btn-primary" type="button" onclick="adjuntar('<?php echo $id ?>','<?php echo $result['id_doc'] ?>','<?php echo $result_com['id_com']?>','<?php echo $contratista ?>','<?php echo $mandante ?>','<?php echo $contrato ?>')"><i style="color: ;font-size: 20px;" class="fa fa-paperclip" aria-hidden="true"></i></button>-->
                                                            <div  class="fileinput fileinput-new" data-provides="fileinput">
                                                                 ...
                                                                 <span  class="fileinput-exists">Cambiar</span><input  type="file" id="carga_dm_trabajador<?php echo $i ?>" name="carga_dm_trabajador<?php echo $i ?>" accept="pdf" /></span>                                                             
                                                             ...
                                                             <button style="background: #1CB394;border:#1CB394;color: #000;" title="Cargar Archivo" class="ladda-button btn-success btn btn-md " data-style="expand-right" type="button" onclick="cargar_doc_mensual('<?php echo $id ?>','<?php echo $result['id_doc']  ?>','<?php echo $result_com['id_com']  ?>','<?php echo $contratista  ?>','<?php echo $mandante  ?>','<?php echo $contrato  ?>','<?php echo $i ?>')"><i class="fa fa-upload" aria-hidden="true"></i></button>   
                                                        </td>
                                                        
                                                        
                                                        <!-- estado -->         
                                                        <?php  if ($result_com['doc']==$result_d['documento']) {
                                                  ...
                                                  # sino documento no esta verificado                                                         
                                                          ...
                                                          <td style="text-align:center">
                                                              ...
                                                              <div  class="fileinput fileinput-new" data-provides="fileinput">
                                                                 ...
                                                                 <span  class="fileinput-exists">Cambiar</span><input disabled=""  type="file" id="carga_dm_trabajador<?php echo $i ?>" name="carga_dm_trabajador<?php echo $i ?>" accept="pdf" /></span>                                                             
                                                                 <a title="Quitar" href="#" class="close fileinput-exists" data-dismiss="fileinput" style="float: none"> <i class="fa fa-times-circle" aria-hidden="true"></i></a>
                                                               </div>
                                             ...
                                             # si documento no existe        
                                                    ...
                                                    <!-- documento -->
                                                    <td  style=""><?php echo $result_d['documento'] ?></td>
                                                   ...
                                                   <td style="text-align:center">
                                                    <!--<button class="btn btn-sm btn-primary" type="button" onclick="adjuntar('<?php echo $id ?>','<?php echo $result['id_doc'] ?>','<?php echo $result_com['id_com'] ?>','<?php echo $contratista ?>','<?php echo $mandante ?>','<?php echo $contrato ?>')"><i style="color: ;font-size: 20px;" class="fa fa-paperclip" aria-hidden="true"></i></button>-->
                                                        ...
                                                        <div  class="fileinput fileinput-new" data-provides="fileinput">
                                                         ...
                                                         <button style="background: #F8AC59;color: #000;border:#F8AC59" title="Cargar Archivo" class="ladda-button btn-success btn btn-md " data-style="expand-right" type="button" onclick="cargar_doc_mensual('<?php echo $id ?>','<?php echo $result_d['documento']  ?>','<?php echo $result_mensual['contrato']  ?>','<?php echo $mensual ?>','<?php echo $result_mensual['mandante']  ?>','<?php echo $result_mensual['mes']  ?>','<?php echo $i  ?>')"><i class="fa fa-upload" aria-hidden="true"></i></button>
                                                   </td>
