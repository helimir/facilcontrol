...
if (isset($_SESSION['usuario'])) { 
   ...
   $query=mysqli_query($con,"select * from doc_desvinculaciones where contratista='$contratista' and mandante='$mandante' ");
   ...
   if ($existe==0) { 
        ...
        if ($estado==1) {
          ...
          $query_notificaciones=mysqli_query($con,"insert into notificaciones (item,nivel,envia,recibe,mensaje,accion,fecha,usuario,url,tipo,mandante,contratista) values ('$item','$nivel','$envia','$recibe','$mensaje','$accion','$date','$usuario','$url','$tipo','$mandante','$idcontratista') ");
          ...
          $recibe=$mandante;
          $tipo=0;
          $url="list_contratistas_mandantes.php";
          $accion="Gestion de Constratista";
          $mensaje="Contratista <b>$nom_contratista</b>  ha sido acreditada.";
          $query_notificaciones=mysqli_query($con,"insert into notificaciones (item,nivel,envia,recibe,mensaje,accion,fecha,usuario,url,tipo,mandante,contratista) values ('$item','$nivel','$envia','$recibe','$mensaje','$accion','$date','$usuario','$url','$tipo','$mandante','$idcontratista') ");
         
          $sql=mysqli_query($con,"insert into doc_observaciones (contratista,mandante,verificados,estado,creado,user) values ('$contratista','$mandante','$veri','$estado','$date','$user')   ");
          
          # guardar servidor y en sefora
          
          /*Datos de conexion a la base de datos*/          
          
          $ruta1='../doc/contratistas/'.$contratista.'/';
          $ruta2='../doc/validados/contratistas/'.$contratista.'/';
          
          # sefora
          #$ruta_s='facil_control/validados/'.$mandante.'/'.$contratista.'/';;
          #$ftp_server = "ftp.sefora.cl";
          #$conn_id = ftp_connect($ftp_server);
          #$login_result = ftp_login($conn_id, "seforacl", "@Pasajero22");
                 
          if (!file_exists($ruta2)) {
                        mkdir($ruta2, 0777, true);
          }
          
          #if (ftp_mkdir($conn_id,$ruta_s)) {
                        #mkdir($ruta_s, 0777, true);
          #}
          ...
          foreach ($documentos as $row) {  
                ...
                # copiar en sefora
                #$archivo_copiar_s= str_replace($ruta1, $ruta_s, $archivo);
                #copy($archivo, $archivo_copiar_s);                
         }
        ...
        # sino estan todos verificados   
            ...
            if ($sql) {    
                ...
                $contador=0;
                $query_doc_contratista=mysqli_query($con,"select doc_contratista from contratistas where id_contratista='$contratista' ");
                ...
                $doc_contratista=unserialize($result_doc_contratista['doc_contratista']);
                    ...
                    for ($i=0;$i<=$total-1;$i++) {
                      if ($doc[$i]!="" and $com[$i]!="") {                         
                        $query_com=mysqli_query($con,"insert into doc_comentarios (id_dobs,doc,comentarios,creado,user) values ('$id_dobs','".$doc[$i]."','".$com[$i]."','$date','$user') ");
...
#### actualizar    
        ...
        $query_doc_very=mysqli_query($con,"select verificados from doc_observaciones where contratista='$contratista' ");
           ...
           for ($i=0;$i<=$total-1;$i++) {
              ...
              if ($existe_estado_comentario==0 and $estado!=1) {
                  if ($doc[$i]!="" and $com[$i]!=""  ) {                
                   ...
                   $query_com=mysqli_query($con,"insert into doc_comentarios (id_dobs,doc,comentarios,creado,user) values ('".$result['id_dobs']."','".$doc[$i]."','".$com[$i]."','$date','$user') ");
