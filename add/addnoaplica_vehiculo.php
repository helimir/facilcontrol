...
$query_guardar=mysqli_query($con,"insert into noaplica_vehiculo (contratista,mandante,documento,mensaje,contrato,vehiculo,siglas,control) values ('$contratista','$mandante','$doc','$mensaje_na','$contrato','$trabajador','$rut','$control') ");
...
if ($query_guardar) {
    ...
    $query_t=mysqli_query($con,"select * from autos where id_auto='$trabajador' ");
    $result_t=mysqli_fetch_array($query_t);
    $nom_trabajador=$result_t['tipo'].' '.$result_t['marca'].' '.$result_t['modelo'].' '.$result_t['year'].' '.$result_t['siglas'].'-'.$result_t['control'];

    $query_doc=mysqli_query($con,"select * from doc_autos where id_vdoc='$doc'  ");
    ...
    $item='Documento Vehiculo No Aplica';                 
    $nivel=3;
    $tipo=3;
    $envia=$contratista;
    $recibe=$_SESSION['mandante'];
    $mensaje="El contratista <b>$nom_contratista</b> envio el documento <b>$documento</b> del vehículo <b>$nom_trabajador</b>, contrato <b>$nom_contrato</b>, indicando que no aplica.";
    ...
    $query_na=mysqli_query($con,"select id_na from noaplica_vehiculo where contrato='$contrato' and vehiculo='$trabajador' and documento='".$doc."' ");
    $result_na=mysqli_fetch_array($query_na);
    $no_aplica=$result_na['id_na'];

    
    mysqli_query($con,"delete from notificaciones where contratista='$contratista' and mandante='$mandante' and contrato='$contrato' and control='$documento' and trabajador='$trabajador' and tipo='3'  ");
    ...
    $nombre=$documento.'_'.$siglas.'.pdf';
    $url='doc/temporal/'.$mandante.'/'.$contratista.'/contrato_'.$contrato.'/vehiculos/'.$siglas.'/'.$nombre;        
    ...
    class PDF extends FPDF {
        ...
        function mensaje($mensaje_na,$nom_contratista,$nom_mandante,$documento,$nom_trabajador) {
            ...
            $this->SetXY(10,45);
            $this->SetFont('Arial','B',10);        
            $this->MultiCell(0,4,utf8_decode('Vehículo: '),0,'L',false);
            $this->SetXY(35,45);
            $this->SetFont('Arial','',10);     
            $this->MultiCell(0,4,utf8_decode($nom_trabajador),0,'L',false);
    ...
    $pdf = new PDF();
    $pdf->AddPage();
    $pdf->mensaje($mensaje_na,$nom_contratista,$nom_mandante,$documento,$nom_trabajador);
    #$pdf->Output('I','no_aplica.pdf'); 
    $pdf->Output('F',$carpeta.$documento.'_'.$siglas.'.pdf');
