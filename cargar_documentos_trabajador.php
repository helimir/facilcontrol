...
if (isset($_SESSION['usuario']) ) {    
    ...
    foreach ($doc as $row) { 
        ...
        if ($_FILES["carga_doc_t"]["name"][$i]) {  
           ...
           $query_na=mysqli_query($con,"select * from noaplica_trabajador where documento='".$row."' and contrato='$contrato' and trabajador='$trabajador' ");
           $result_na=mysqli_num_rows($query_na);
           #existe un documento no aplica
           if ($result_na>0) {
               #borrar registro del documento en no apllica trabajar
               mysqli_query($con,"delete from noaplica_trabajador where documento='".$row."' and contrato='$contrato' and trabajador='$trabajador' ");
               #borrar la notidicacion no aplica
               mysqli_query($con,"delete from notificaciones where documento='".$row."' and contrato='$contrato' and item='Documento Trabajador No Aplica' and trabajador='$trabajador' ");
           }
                        
           #$extension = pathinfo($arch, PATHINFO_EXTENSION);
                
           $carpeta = 'doc/temporal/'.$_SESSION['mandante'].'/'.$contratista.'/contrato_'.$contrato.'/'.$rut_t.'/';
           $nombre=$result_doc['documento'].'_'.$rut_t.'.pdf';
                ...
                if (@move_uploaded_file($_FILES["carga_doc_t"]["tmp_name"][$i], $archivo)) {   
                    ...
                    $query_noti_existe=mysqli_query($con,"select * from notificaciones where procesada='0' and item='Documento Trabajador Recibido' and control='".$result_doc['documento']."' and contratista='$contratista' and mandante='".$_SESSION['mandante']."' and contrato='$contrato' and trabajador='$trabajador'  ");
                    $existe_noti=mysqli_num_rows($query_noti_existe);

                    $query_noti_existe_o=mysqli_query($con,"select * from notificaciones where procesada='0' and item='Observacion Documento Trabajador' and control='".$result_doc['documento']."' and contratista='$contratista' and mandante='".$_SESSION['mandante']."' and contrato='$contrato'  ");
                    $existe_noti_o=mysqli_num_rows($query_noti_existe_o);

                    mysqli_query($con,"update comentarios set leer_contratista=1,leer_mandante=1, estado='1' where doc='".$result_doc['documento']."' and contratista='$contratista' and mandante='".$_SESSION['mandante']."' and trabajador='$trabajador' ");
                    ...
                    if ($existe_noti=='0') {
                        ...
                        mysqli_query($con,"insert into notificaciones (item,nivel,envia,recibe,mensaje,accion,fecha,usuario,url,tipo,mandante,contratista,control,contrato,perfil,cargo,trabajador,documento) values ('$item','$nivel','$envia','$recibe','$mensaje','$accion','$date','$usuario','$url','$tipo','".$_SESSION['mandante']."','$contratista','$nom_documento','$contrato','$perfil','$cargo','".$_SESSION['trabajador']."','$nom_documento') ");                                
                        mysqli_query($con,"update comentarios set leer_contratista=1,leer_mandante=1, estado='1' where  doc='".$result_doc['documento']."' and contratista='$contratista' and mandante='".$_SESSION['mandante']."' and trabajador='$trabajador' ");
                        mysqli_query($con,"delete from notificaciones  where contrato='$contrato' and contratista='$contratista' and item='Observacion Documento Trabajador' and control='".$result_doc['documento']."' and trabajador='$trabajador'");  
                        mysqli_query($con,"delete from notificaciones  where contrato='$contrato' and contratista='$contratista' and item='Documento trabajador' and control='".$result_doc['documento']."' and trabajador='$trabajador' "); 
    ...
    foreach ($doc_e as $row_e) {
        ...
        //if ($existe_doc[$i]==1) {
                    ...
                    $query_doc_e=mysqli_query($con,"select * from doc where id_doc='".$row_e."' ");
                    $result_doc_e=mysqli_fetch_array($query_doc_e);                                   
                                
                    $ruta1= 'doc/trabajadores/'.$contratista.'/'.$rut_t.'/';
                    $ruta2= 'doc/temporal/'.$_SESSION['mandante'].'/'.$contratista.'/contrato_'.$contrato.'/'.$rut_t.'/';
                    ...
                    $nombre_e=$result_doc_e['documento'].'_'.$rut_t.'.pdf';
                    ...
                    if (copy($archivo,$archivo_copiar)) {
                      ...
                      $copiado=0;  
                            ...
                            $envia=$contratista;
                            $recibe=$_SESSION['mandante'];
                            $mensaje="El contratista <b>$nom_contratista</b> envio el documento <b>$nombre_documento</b> del trabajador <b>$nom_trabajador</b>, contrato <b>$nom_contrato</b> para ser revisado.";
                            $usuario=$_SESSION['usuario'];
                            $accion="Revisar documento de contratista";
                            $url="verificar_documentos_trabajador_mandante.php";
