...
<head>
    ...
    <title>FacilControl | Gestión de Vehículos</title> 
    <!-- Favicons -->
        ...
        <div id="page-wrapper" class="gray-bg">
           ...
           <?php include('superior.php') ?> 
            ...
            <div style="" class="row wrapper white-bg ">
                <div class="col-lg-10">
                    <h2 style="color: #010829;font-weight: bold;">Gestion de Vehículos: [<?php echo $result_o['nombre_contrato']  ?>]</h2>
                </div>
            </div>
                ...
                <div class="col-lg-12">
                    <div class="ibox ">
                            <div class="ibox-title">
                              ...
                              <div class="form-group row">
                                    <div class="col-lg-12 col-sm-12 col-sm-offset-2">
                                        <a class="btn btn-sm btn-success btn-submenu" href="#" class="" type="button"><i  class="fa fa-chevron-right" aria-hidden="true"></i> Vehículos Acreditados</a>
                        ...
                        <div class="ibox-content">
                            <input class="form-control form-control-sm m-b-xs" id="filter" placeholder="Buscar un Vehículo">
                            
                            <div class="table-responsive">
                                <table class="table footable " data-page-size="10" data-filter="#filter">
                                   <thead style="background:#010829;color:#fff">
                                    <tr style="font-size: 12px;">
                                        <th style="width: 25%;border-right:1px #fff solid;">Vehículo</th>
                                        <th style="width: 10%;border-right:1px #fff solid;">Patente</th>
                                        <th style="width: 15%;border-right:1px #fff solid;" >Sigla</th>
                                        <th style="width: 15%;border-right:1px #fff solid;">Perfil de Vehículo</th>
                                        <th style="width: 20%;text-align: center;" >Acreditación</th>
                                    ...
                                    <?php
                                      ...
                                      $query_ta=mysqli_query($con,"select a.*, t.*, o.perfiles_v from vehiculos_asignados as a LEFT JOIN autos as t ON t.id_auto=a.vehiculos LEFT JOIN perfiles_vehiculos as p ON p.contrato=a.contrato LEFT JOIN contratos as o ON o.id_contrato=a.contrato  where a.contrato='".$_SESSION['contrato']."' and a.mandante='".$_SESSION['mandante']."' and a.estado!='2' ");
                                      $result_ta=mysqli_fetch_array($query_ta);
                                      $cargos=unserialize($result_ta['vehiculos']);
                                      $perfiles=unserialize($result_ta['perfiles_v']);
                                      ...
                                      foreach ($query_ta as $row) {
                                        ...
                                        foreach ($perfiles as $row_c) {
                                            #if ($row_c==$perfiles[$j]) {
                                                $query_pc=mysqli_query($con,"select * from perfiles where id_perfil='".$row['perfiles']."' ");
                                        ...
                                        ?>
                                         <tr>
                                            ...
                                            <!-- trabajador -->
                                            <td><?php echo $row['tipo'].' '.$row['marca'].' '.$row['modelo'].' '.$row['year'] ?></td>
                                            ...
                                            <td style="text-align: center;">
                                             <?php if ($row['verificados']==0) { ?>    
                                                    <button title="Vehículo No Validado" class="btn btn-danger btn-xs btn-block" onclick="doc_validar(<?php echo $row['vehiculos'] ?>,<?php echo $row['cargos'] ?>,<?php echo $result_pc['id_perfil'] ?>)"><strong>GESTI&Oacute;N DOCUMENTOS</strong></button>
                                             <?php } else { 
                                                      if ($row['verificados']==1) { ?> 
                                                        <button title="Vehículo Validado" class="btn btn-success btn-xs btn-block" onclick="doc_validar(<?php echo $row['vehiculos'] ?>,<?php echo $row['cargos'] ?>,<?php echo $result_pc['id_perfil'] ?>)"><strong>VALIDADO</strong></button>
                                                <?php } else {   ?>
                                                        <button title="Vehículo Validado" class="btn btn-warning btn-xs btn-block text-dark" onclick="doc_validar(<?php echo $row['vehiculos'] ?>,<?php echo $row['cargos'] ?>,<?php echo $result_pc['id_perfil'] ?>)"><strong>REVISI&Oacute;N EN PROCESO</strong></button>    
                                                <?php }    
                                                   } ?>
                                            </td>
